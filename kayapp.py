...
def theme_css(theme: str) -> str:
    ...
    surface   = "#181818" if dark else "#ffffff"
    card_bg   = "#212121" if dark else "#ffffff"
    ...
    <style>
      * {{ box-sizing:border-box; margin:0; padding:0; -webkit-tap-highlight-color:transparent; }}
      ...
      .stButton>button {{ background:{btn_bg}; color:{btn_fg}; border:1px solid {border}; border-radius:10px; }}
      ...
      .stFileUploader>div>div {{ background:{surface}; border:1px solid {border}; border-radius:12px; }}
      .stExpander, .stExpander>details {{ background:{surface}; border:1px solid {border}; border-radius:12px; }}
...
class LateralRaiseProcessor(VideoProcessorBase):
    def __init__(self, bg_gray=255):
        ...
        # FSM (หลัก)
        self.rep_phase = 0  # 0: waiting (Standing) / 1: in-rep (NonStanding → รอ Standing)
        ...
        # ตรวจ “ทะลุเส้นคอ” และแสดง Bad แบบค้างจนเริ่มรอบต่อไป
        self.phase_crossed_neck = False
        self.bad_latch = False   # <<==== LATCH สำคัญ
    ...
    def _process_logic(self, image_bgr):
        ...
        if results.pose_landmarks:
            ...
            wrist_above_neck = (lw_y < moving_eye_line_y) or (rw_y < moving_eye_line_y)

            # มุมหัวไหล่
            l_shoulder = [lm[12].x, lm[12].y]; l_elbow = [lm[14].x, lm[14].y]; l_hip = [lm[24].x, lm[24].y]
            left_shoulder_angle  = calculate_angle(l_elbow, l_shoulder, l_hip)
            r_shoulder2 = [lm[11].x, lm[11].y]; r_elbow2 = [lm[13].x, lm[13].y]; r_hip = [lm[23].x, lm[23].y]
            ...
            if main_status == "Standing":
                self.standing_streak += 1; self.nonstanding_streak = 0
            else:
                self.nonstanding_streak += 1; self.standing_streak = 0
            ...
            if self.standing_streak >= self.STANDING_CONFIRM:
                ...
                self.stable_main = "NonStanding"

            # ===== FSM นับรอบ Standing -> NonStanding -> Standing (พร้อมคอ/neck + latch) =====
            if self.stable_main is not None:
                if self.rep_phase == 0:
                    if self.prev_stable_main == "Standing" and self.stable_main == "NonStanding":
                        self.rep_phase = 1
                        self.phase_crossed_neck = wrist_above_neck  # เริ่มรอบ รีเซ็ต/ตั้งจากเฟรมแรก
                        self.bad_latch = False                      # เคลียร์ BAD latch เมื่อเริ่ม rep ใหม่

                elif self.rep_phase == 1:
                    # อัปเดตสถานะเคยทะลุเส้นคอหรือยัง
                    if wrist_above_neck:
                        self.phase_crossed_neck = True

                    if self.stable_main == "Standing":
                        # จบรอบ
                        self.count += 1
                        if not self.phase_crossed_neck:
                            # ยืน->ยก->ยืน โดย "ไม่เคย" ทะลุเส้นคอ => Bad และค้างไว้
                            self.bad_latch = True
                        self.rep_phase = 0

            # ===== ตัดสินฟอร์มพื้นฐาน (ของเดิม) =====
            ...
            # ตรวจ Standing->Moving แล้วมุมลด
            ...
            # ===== เอาต์พุตฟอร์มแบบมี LATCH =====
            # ถ้าเคยถูกจับ Bad ในรอบก่อนหน้าและยังไม่เริ่มรอบใหม่ -> บังคับให้โชว์ Bad
            display_form = "Bad" if self.bad_latch else form_text

            # Debug overlay (เอาออกได้)
            x_left, y_base, y_step = 10, 20, 22

            col_form = (0,200,0) if display_form in ("Good","Normal") else (0,0,255)
            ...
            # อัปเดตค่าให้ UI ด้านขวา (ใช้ค่าที่ผ่าน LATCH แล้ว)
            self.form_text = display_form
